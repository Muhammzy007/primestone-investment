...
const generateToken = (userId, role, username) => jwt.sign({ userId, role, username }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/register', [
  ...
  try {
    ...
    const { data: newUser, error } = await supabase.from('users').insert([{ username, email, password_hash: hashedPassword, role: 'user' }]).select().single();
    if (error) throw error;
    const token = generateToken(newUser.id, 'user', username);
    await emailService.sendWelcomeEmail(email, username);
    res.json({ success: true, data: { token, user: { id: newUser.id, username, email, role: 'user' } } });
  } catch (error) {
...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    const { data: user, error } = await supabase.from('users').select('id, username, email, role, created_at').eq('id', decoded.userId).single();
